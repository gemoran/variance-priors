...
double update_sigma(double *r, int n);
...
SEXP scaledSSL_gaussian(SEXP X_, SEXP y_, SEXP penalty_, SEXP variance_, SEXP lambda1_, SEXP lambda0s_, SEXP theta_, SEXP sigma_, SEXP min_sigma2_, SEXP a_,  SEXP b_, SEXP eps_, SEXP max_iter_, SEXP counter_) {
  ...
  double sigma = REAL(sigma_)[0];
  double sigma_init = REAL(sigma_)[0];
  double min_sigma = sqrt(REAL(min_sigma2_)[0]);
  double aa = REAL(a_)[0];
  double bb = REAL(b_)[0];
  ...
  for (int l = 0; l < L; l++) {
    ...
    if (l != 0) {
      ...
      thresholds[l] = threshold(theta, sigma, lambda1, lambda0, n);
        ...
        while (INTEGER(iter)[l] < max_iter) {
          ...
          for (int j=0; j<p; j++) {
            ...
            if (e1[j]) {
              ...
              b[l*p+j] = SSL(z[j], a[j], lambda0, lambda1, theta, 1, n, delta, sigma);
            ...
            if (counter == count_max){
              ...
              if(strcmp(penalty, "adaptive")==0) {
                ...
                delta = threshold(theta, sigma, lambda1, lambda0, n); 
              }
